...
class ClipMultilabelLoss(nn.Module):
    ...
    def get_ground_truth(self, device, num_logits, raw_texts, related_texts) -> torch.Tensor:
        # calculated ground-truth and cache if enabled
        if self.prev_num_logits != num_logits or device not in self.labels:
    ...
    def forward(self, image_features, text_features, raw_texts, related_texts, logit_scale, output_dict=False):
        device = image_features.device
        logits_per_image, logits_per_text = self.get_logits(image_features, text_features, logit_scale)
